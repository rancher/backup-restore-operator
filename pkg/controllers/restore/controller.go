...
func (h *handler) OnRestoreChange(_ string, restore *v1.Restore) (*v1.Restore, error) {
	...
	updateErr := retry.RetryOnConflict(retry.DefaultRetry, func() error {
		...
		restore.Status.Conditions = []k8sv1.Condition{}
		v1.RestoreConditionReady.ToFluentBuilder(restore).True().SetMessage("Completed").Apply(restore)
...
func (h *handler) setReconcilingCondition(restore *v1.Restore, originalErr error) (*v1.Restore, error) {
	...
	err := retry.RetryOnConflict(retry.DefaultRetry, func() error {
		...
		v1.RestoreConditionReconciling.ToFluentBuilder(updRestore).SetError("", originalErr).Apply(updRestore)
		v1.BackupConditionReady.ToFluentBuilder(updRestore).SetMessage("Retrying").Apply(updRestore)
